 ...
 * numbered_list_item, to_do, quote, callout, divider, image, code.
 * 지원하지 않는 블록(테이블, 임베드, 데이터베이스, 동기화 블록 등)은 조용히
 * 건너뛴다 — 완벽한 노션 클론이 아니라 "텍스트·이미지·기본 서식" 위주의 칼럼
 * 콘텐츠가 목적이라 이 정도로 충분하다고 봄.
 */
...
async function blocksToHtml(blocks) {
  ...
  while (i < blocks.length) {
    ...
    if (type === "toggle") {
      // 접기/펼치기 상호작용까지는 지원하지 않고, 제목을 굵게 + 내용은 펼쳐서 보여줌
      let html = `<p><strong>${richTextToHtml(block.toggle.rich_text)}</strong></p>`;
      if (block.__children && block.__children.length) html += await blocksToHtml(block.__children);
      parts.push(html);
      continue;
    }
