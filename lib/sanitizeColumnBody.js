...
// 직접 입력한 글자 크기(px)만 예외적으로 style 속성을 허용한다 — 딱 이 정규식 하나만
// 통과시키고, 그 외 어떤 CSS 속성/값도 style로는 못 들어가게 막는다(자유 style은
// CSS 인젝션 통로라 절대 그대로 허용하지 않음). 8~96px 범위로 값도 clamp.
function sanitizeFontSizeStyle(value) {
  const m = /^font-size:\s*(\d{1,3})px;?$/i.exec(String(value == null ? "" : value).trim());
  if (!m) return null;
  const n = Math.max(8, Math.min(96, parseInt(m[1], 10) || 0));
...
function serializeAttrs(name, parsed) {
  ...
  allowedAttrNames.forEach((attrName) => {
    ...
    if (attrName === "style" && name === "span") {
      const safeStyle = sanitizeFontSizeStyle(value);
