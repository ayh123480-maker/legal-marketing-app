...
- 같은 프로젝트를 여러 번 Notion에 업로드할 때 기존 페이지 **내용을 실제로
  갱신(update)**하는 것은 여전히 범위 밖 — Notion API로 기존 페이지 children을
  안전하게 교체하려면 기존 블록 삭제·재작성이 필요하고, 사용자가 노션에서
  직접 고친 내용을 덮어쓸 위험이 있어서 범위가 커진다.
  ~~다만 "매번 새 페이지가 생겨서 혼란스럽다"는 문제 자체~~는 2026-09-17에
  안전한 범위로 완화함: `notionPageId`가 이미 있는(예전에 한 번 업로드한)
  프로젝트를 열면, 기본 CTA가 "이미 업로드한 노션 페이지 열기"로 바뀌고
  "노션에 업로드" 버튼은 "🔁 새 페이지로 다시 업로드"로 문구가 바뀌면서
  "별도의 새 페이지가 하나 더 생긴다"는 걸 명확히 알려준다(`notionPageId`만으로
  `https://www.notion.so/<id>` 링크를 만들 수 있어서 URL을 따로 저장할 필요는
  없었음). 페이지 내용을 실제로 갱신하는 기능 자체는 여전히 다음 작업.
